...
class Chinese_Lesson_Model_Jpexetype extends Mage_Core_Model_Abstract
{
	...
	protected function _construct()
	{
		...
		// @todo hardcode assign to lesson 27, need to change to dynamic later
		$this->lesson_id = 27;
		
		// Initialise collection filtered by lesson_id
	...
	public function getFirstTypePath(){
		...
		return $this->_exeBaseDir() . 'type5.php';
	}
	...
	public function getSourceArr($type_arr,$typeId) {
     ...
     /**
	 * Get requested audio url array with source value only for specific type
	 * @param $typeId
	 * @return Array
	 */
	public function getAudioArr($type_arr,$typeId) {
		if((INT)$typeId==2) {
	        $type_audio = array();
            foreach($type_arr as $typeArr) {
	            array_push($type_audio, $this->getTypeResource($typeArr['id'], 1));
            }
            return $type_audio;
	     }
	}
